...
const ChildItem: React.FC<ChildItem> = (props) => {
  const { index, linkMap, cTimeKey = 'createTime', tNameKey = 'traceName', item, down, log } = props;
  const { lastIcon, callIcon, downIcon, upIcon, firstIcon, normalIcon } = IMGSTR;
  const call = () => { //这个部分会和原生的交互，后续可以加入
  ...
  return (
    ...
    <div className={`${prefixCls}-node`}>
      <img src={index === 0 ? firstIcon : normalIcon} className={`${prefixCls}-node-img`} />
      <div className={`${prefixCls}-node-time`}>
        {cTimeKey}
        <img src={lastIcon} alt='' className={`${prefixCls}-last-icon`} hidden={index !== 0} />
      </div>
      <div className={`${prefixCls}-child`}>
        <div className={`${prefixCls}-child-block`}>
          <div className={`${prefixCls}-child-title`}>{tNameKey}</div>

          <img src={openFlag ? upIcon : downIcon} alt='' onClick={() => {
            updateFlag(!openFlag);
            log('ProcessTrack')
          }
              ...
              linkMap.map((childItem: any) => {
                ...
                return (
                  <div className={`${prefixCls}-child-content`} key={key}>
                    ...
                    <div
                      className={item[key] ? `${prefixCls}-child-value` : `${prefixCls}-hide`}
