...
export const getTransform = (
    ...
    const webCores = ['transform', 'MozTransform', 'WebkitTransform', 'OTransform'];
    const webTransform = {};
...
const sliderLeftStyle = (state: 'open' | 'close', startOffset: number | string) => { 
    return {
        top: 0,
        left: startOffset,
        bottom: 0,
...
const sliderRightStyle = (state: 'open' | 'close', startOffset: number | string) => { 
    return {
        top: 0,
        right: startOffset,
        bottom: 0,
...
export const bemStyle =
    (mode: TransitionType['mode'], state: 'open' | 'close', startOffset: number | string):
...
export const useTimeout = (
    ...
    const ms = Array.isArray(duration) ? (show ? duration[0] : duration[1]) : duration;
